//! Synthetic image-output fixtures (dev/test only).
//!
//! There is no real image directory mounted in development, so tests and manual
...
/// ```text
/// <source>/
///   Image_00001_.png        (a) numbered  + (d) prompt graph -> meta extracts pos/neg
///   Image_00002_.png        (a) numbered  + plain (no text chunk) -> meta empty
///   Image_00003_.png        (a) numbered  + (d) broken prompt JSON -> raw-only
///   2026-06-01/Image_00001_.png, _00002_.png   (b) date subfolder
///   proj-a/Image_00009_.png                     (c) arbitrary-named subfolder
...
pub fn build(source: &Path) -> io::Result<()> {
    ...
    write_png(
        &source.join("Image_00001_.png"),
        Some(("prompt", SAMPLE_GRAPH)),
    )?;
    write_png(&source.join("Image_00002_.png"), None)?;
    write_png(
        &source.join("Image_00003_.png"),
        Some(("prompt", "{ this is not valid json")),
    )?;

    // (b) date subfolder.
    write_png(&source.join("2026-06-01/Image_00001_.png"), None)?;
    write_png(&source.join("2026-06-01/Image_00002_.png"), None)?;

    // (c) arbitrary-named subfolder.
    write_png(&source.join("proj-a/Image_00009_.png"), None)?;
...
/// before IDAT, matching where image generators write their `prompt`/`workflow`).
